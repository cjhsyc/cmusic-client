...
// 更新用户信息
export const updateUserMsg = (params: URLSearchParams) =>
  request.put(`user`, params)
export const updateUserPassword = (params: URLSearchParams) =>
...
export const getSingerOfSex = (sex: number) =>
  request.get(`singer/sex/detail/${sex}`)

// =======================> 歌曲 API
// 返回指定歌曲ID的歌曲
export const getSongOfId = (id: string) => request.get(`song/detail?id=${id}`)
// 返回指定歌手ID的歌曲
export const getSongOfSingerId = (id: string) =>
  request.get(`song/singer/detail?singerId=${id}`)
// 返回指定歌手名的歌曲
export const getSongOfSingerName = (keywords: string) =>
  request.get(`song/singerName/detail?name=${keywords}`)
// 下载音乐
export const downloadMusic = (url: string) =>
  request.get(url, { responseType: 'blob' })

// =======================> 收藏 API
// 返回的指定用户ID的收藏列表
export const getCollectionOfUser = (userId: string) =>
  request.get(`collection/${userId}`)
// 添加收藏的歌曲 type: 0 代表歌曲， 1 代表歌单
export const setCollection = (params: URLSearchParams) =>
  request.post(`collection`, params)
//删除
...
//收藏状态
export const collectionIs = (params: URLSearchParams) =>
  request.post(`collection/status`, params)

// =======================> 评分 API
// 提交评分
export const setRank = (params: URLSearchParams) => request.post(`rank`, params)
// 获取指定歌单的评分
export const getRankOfSongListId = (songListId: string) =>
  request.get(`rank/${songListId}`)
// 获取指定用户的歌单评分
